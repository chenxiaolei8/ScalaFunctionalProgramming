...
object AnonymousFunctions {
  ...
  def main(args: Array[String]): Unit = {
    println(formatResult("absolute value", -42, abs))//绝对值
    println(formatResult("factorial", 7, factorial))//阶乘
    println(formatResult("increment", 7, (x: Int) => x + 1))//匿名方法
    println(formatResult("increment2", 7, (x) => x + 1))//匿名方法缺省数据类型
    println(formatResult("increment3", 7, x => x + 1))//匿名方法缺省数据类型
    println(formatResult("increment4", 7, _ + 1))//相加
    println(formatResult("increment5", 7, x => { val r = x + 1; r }))//匿名方法实现方式
  }
}
